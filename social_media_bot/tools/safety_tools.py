...
class SafetyChecker(BaseTool):
    ...
    def _run(self, content: str) -> Dict:
        try:
            prompt = f"""You are a content safety expert. Analyze this content for safety concerns:
...
class DuplicateDetector(BaseTool):
    ...
    def _run(self, new_content: str, existing_content: list) -> Dict:
        try:
            import json
...
class ComplianceChecker(BaseTool):
    ...
    def _run(self, content: str, platform: str) -> Dict:
        try:
            prompt = f"""You are a platform policy expert. Review this content for compliance with {platform} policies:
